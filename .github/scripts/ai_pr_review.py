...
5. Generate blockViolations of CodingStandards for the pull request: FOR EACH LINE OF CODE added in the Pull Request (lines starting with `+`, These are added lines), go over ALL the CodingStandards Rules (every rule has a special Rule_X xml element where X represents a number starting from 1. Each rule consists of Description_X, BadCodeExample_X, GoodCodeExample_X xml elements, X is a numbering that matches the numbering of Rule_X, the Description_X xml element describes the rule (we call it the 'rule description'), BadCodeExample_X xml element consists of a code example that violates the 'rule description', GoodCodeExample_X xml element consists of a code example that follows the 'rule description') and report ALL the violations (rules the line added does not follow) these added line introduces. After reporting all the violations for the line added, analyse the violations and the line added and suggest a fix that will solve all the violations found for the line added, MAKE SURE to not violating any of the Rules in CodingStandards with the suggested fix. The suggested fix should be in code (DO NOT wrap the code with triple backticks (```) or the coding language, e.g. do not write ```javascript ``` nor ```python ```) in the same coding language of the file (you can infer the coding language from the extension of the file) and should NOT include any comments. Feel free to use the rule's code examples. MAKE SURE TO CHECK FOR VIOLATIONS FOR EVERY SINGLE LINE OF CODE ADDED. Your analysis must be thorough, aiming to identify ALL violations according to the provided CodingStandards. Return the EXACT line number in the file where the violation PRIMARILY occurs—this must be a single integer. Carefully verify that the line you select is the true origin of the issue, not just a line affected by it. Double-check whether the violation actually begins on the preceding line, as it’s common to mistakenly choose the line after the problem has already started. Your answer must reflect the line where the violation is primarily introduced, not where its symptoms appear. For example, if a variable is used without being initialized and that usage is on line 6, but the missing initialization should have happened on line 5, then line 5 is the correct line to return—not line 6. The output should be an array of JSON objects (one for each new line added where a violation has occured) of the structure:
{{
    "file": "string", // Path to the violated file in which the line added is in.
    "line": "integer", // The EXACT line number in the file where the violation PRIMARILY occurs—this must be a single integer. Carefully verify that the line you select is the true origin of the issue, not just a line affected by it. Double-check whether the violation actually begins on the preceding line, as it’s common to mistakenly choose the line after the problem has already started. Your answer must reflect the line where the violation is primarily introduced, not where its symptoms appear. For example, if a variable is used without being initialized and that usage is on line 6, but the missing initialization should have happened on line 5, then line 5 is the correct line to return—not line 6.
    "violations": [ // Array of strings describing the violation(s) in  detail of a specific line.
        "string" // State the specific rule being violated from the coding standards. Include the example from the coding standards if relevant and helpful for context.
        // ... more violations for the same line/block
    ],
